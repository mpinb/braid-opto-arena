        ...
        if self.expanding:
            ...
            if elapsed < self.duration:
                if self.type == 'linear':
                    ...
                    radius = (2 ** (elapsed / self.duration) - 1) / (2 - 1) * self.max_radius
                pygame.draw.circle(screen, self.color, (self.position, SCREEN_HEIGHT // 2), int(radius))
            else:
                self.expanding = False
...
if __name__ == "__main__":
    ...
    parser.add_argument("config_file", type=str, help="Path to the configuration file (.toml)")
    parser.add_argument("--standalone", action="store_true", help="Run the program in standalone mode without ZMQ")
