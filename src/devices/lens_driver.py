...
class LensDriver:
    def __init__(self, port, debug=False, log_level=logging.INFO):
        ...
        self.logger.info(f"Initializing LensController on port {port}")
    ...
    def get_mode(self):
        mode = "current" if self.mode == 1 else "focal_power" if self.mode == 5 else "unknown"
    ...
    def set_current(self, current):
        ...
        if self.get_mode() != "current":
            self.logger.error(f"Cannot set current when not in current mode. Current mode: {self.get_mode()}")
    ...
    def set_diopter(self, diopter):
        ...
        if self.get_mode() != "focal_power":
            self.logger.error(f"Cannot set focal power when not in focal power mode. Current mode: {self.get_mode()}")
    ...
    def ramp_to_zero(self):
        ...
        if current_mode == "current":
            ...
            self.logger.warning(f"Unknown mode: {current_mode}. Unable to ramp to zero.")
